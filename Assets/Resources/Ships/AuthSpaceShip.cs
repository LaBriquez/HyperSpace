...
public class AuthSpaceShip : MonoBehaviour
{
    public float acceleration, speed, maniability, mass, maxCDBonus;
}

public struct SpaceShip : IComponentData
{
    public int Checkpoint, Tour, Bonus;
    public float Acceleration, MaxSpeed, Maniability, Mass, cdBonus, maxCDBonus, DistCheck, VMultip, 
        BonMaltimer, MMult, AMult;
}

public class SpaceShipBaker : Baker<AuthSpaceShip>
{
    public override void Bake(AuthSpaceShip authoring)
    {
...
public partial struct BotSystem : ISystem
{
    ...
    private NativeArray<float3> positionsMap1, positionsMap2;
    
    // le but étant d'avoir le max d'entité, les burst est important
    [BurstCompile]
    public void OnUpdate(ref SystemState state)
    {
        ...
        var p = SystemAPI.GetSingletonRW<Player>();
        float dt = Time.deltaTime;
        
        foreach (var (transform, s, physic) in 
                 SystemAPI.Query<RefRW<LocalTransform>, RefRW<SpaceShip>, RefRW<PhysicsVelocity>>())
        {
            // si le player n'as pas encore commencer, on ne fait rien
            if (p.ValueRW.TimerBegin > 0)
                return;

            //bouge dans la direction du prochain checkpoint
            physic.ValueRW.Angular = float3.zero;
            float3 direction = positions[s.ValueRW.Checkpoint % positions.Length] - transform.ValueRW.Position;

            float3 directionMove = math.lerp(transform.ValueRW.Forward(), math.normalize(direction), s.ValueRW.Maniability * s.ValueRW.MMult * dt);
            
            //activation des bonus dès que l'on en a un
            ...
            // regarde dans la direction dans laquelle on va
            transform.ValueRW.Rotation = quaternion.LookRotation(directionMove, new (0, 1, 0));

            physic.ValueRW.Linear += s.ValueRW.Acceleration * s.ValueRW.AMult *
            ...
            s.ValueRW.DistCheck = math.lengthsq(direction);
            if (s.ValueRW.DistCheck < 10000)
            {
                s.ValueRW.Checkpoint++;
                if (s.ValueRW.Checkpoint % positions.Length == 0)
                {
                    s.ValueRW.Tour--;
                }
            }
